  ...
  return (dispatch) => {
    return api
      ...
      .then((response) => {
        ...
        result.length > 0 &&
          dispatch(selectPhysiologicalConstant(result[result.length - 1]));
...
export function savePhysiologicalConstant(physiologicalConstant, cb = null) {
  return (dispatch) => {
    if (physiologicalConstant._id) {
      return api
        .putData(
          modelPoint + "/" + physiologicalConstant._id,
          physiologicalConstant
        )
        .then((response) => {
      ...
      return api
        .postData(modelPoint, physiologicalConstant)
        .then((response) => {
          //dispatch(addPhysiologicalConstant(physiologicalConstant));
...
export function deletePhysiologicalConstant(physiologicalConstant, cb) {
  return (dispatch) => {
    return api
      .deleteData(modelPoint + "/" + physiologicalConstant._id)
      .then((response) => {
...
export function getPhysiologicalConstant(id, cb = null) {
  return (dispatch) => {
    if (id) {
      ...
      dispatch(selectPhysiologicalConstant(new PhysiologicalConstant()));

...
const Diseases = (props) => {
  ...
  const rules = (key, value) => {
    switch (key) {
      case "criteria":
        errors[0] =
          value.length > 0 && value.length < 80
            ? "El criterio debe tener mas de 80 carácteres"
            : false;
      ...
      case "observations":
        errors[1] =
          value.length > 0 && value.length < 80
            ? "Las observaciones debe tener mas de 80 carácteres"
            : false;
  ...
  return (
    <Grid lg={12} md={12} xs={12}>
        ...
        <PerfectScrollbar>
          <div className={classes.inner}>
            <Table>
              ...
              <TableBody>
                {props.detectedDiseases.reverse().map((disease) => (
                  <TableRow>
                    <TableCell>{disease.doctorDetails[0].name}</TableCell>
      ...
      >
        <DialogTitle>Enfermedad detectada</DialogTitle>
        <DialogContent>
          <DialogContentText>

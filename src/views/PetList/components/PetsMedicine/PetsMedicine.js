...
const PetsMedicine = (props) => {
  ...
  const [openMedicineConfirmation, setOpenMedicineConfirmation] =
    useState(false);
    ...
    if (data.length == 0) {
      ...
      const filteredArray = products.filter(
        (product) =>
          ...
          (product.administrationWay
            ? product.administrationWay.toLowerCase().includes(data)
            : false) ||
          (product.presentation
            ? product.presentation.toLowerCase().includes(data)
            : false)
      );
  ...
  return (
    <div>
      ...
      >
        <DialogTitle>Desea seleccionar este producto</DialogTitle>
        <DialogContent>
          <DialogContentText>
